...
def first_run(self):
    data = ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Intro to CS')
    conn = sqlite3.connect('db_studentTracking.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email, col_course) VALUES (?,?,?,?,?,?)""")
            conn.commit()
    conn.close()
